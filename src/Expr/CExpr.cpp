...
CExprValuePtr
CExpr::
variableSubscript(const std::string &, const CExprValueArray &) const
{
...
nameType(const std::string &name)
{
  ...
  auto typeChar = name[name.size() - 1];

  if      (typeChar == '%') return CExprValueType::INTEGER;
  else if (typeChar == '$') return CExprValueType::STRING;
